...
MIDDLEWARE = [
    ...
    "config.public_cors.PublicApiCorsMiddleware",
...
MEDIAFILE_MAX_UPLOAD_SIZE = int(env("MEDIAFILE_MAX_UPLOAD_SIZE", str(10 * 1024 * 1024)))
...
PUBLIC_API_CORS_ALLOW_ALL = env_bool("PUBLIC_API_CORS_ALLOW_ALL", True)
...
if CORS_ALLOW_ALL_ORIGINS and CORS_ALLOW_CREDENTIALS:
    raise ImproperlyConfigured("CORS_ALLOW_ALL_ORIGINS cannot be combined with CORS_ALLOW_CREDENTIALS")
if IS_PRODUCTION and CORS_ALLOW_ALL_ORIGINS:
    raise ImproperlyConfigured(
        "CORS_ALLOW_ALL_ORIGINS must be false in production; use PUBLIC_API_CORS_ALLOW_ALL instead"
    )
...
PUBLIC_SITE_DEFAULT_DOMAIN = env(
    "PUBLIC_SITE_DEFAULT_DOMAIN",
    "" if IS_PRODUCTION else "localhost:3000",
).strip()
if IS_PRODUCTION and not PUBLIC_SITE_DEFAULT_DOMAIN:
    raise ImproperlyConfigured("PUBLIC_SITE_DEFAULT_DOMAIN must be set when DJANGO_ENV=production")
PUBLIC_SITE_DEFAULT_URL = env(
    "PUBLIC_SITE_DEFAULT_URL",
    f"http://{PUBLIC_SITE_DEFAULT_DOMAIN}",
).rstrip("/")
parsed_public_site_default_url = urlparse(PUBLIC_SITE_DEFAULT_URL)
if (
    parsed_public_site_default_url.scheme not in {"http", "https"}
    or not parsed_public_site_default_url.hostname
):
    raise ImproperlyConfigured("PUBLIC_SITE_DEFAULT_URL must be an absolute HTTP(S) URL")
if IS_PRODUCTION and parsed_public_site_default_url.hostname in {"localhost", "127.0.0.1", "0.0.0.0"}:
...
TELEGRAM_DELIVERY_MODE = env("TELEGRAM_DELIVERY_MODE", "direct").strip().lower()
TELEGRAM_RELAY_URL = env("TELEGRAM_RELAY_URL", "").strip()
TELEGRAM_RELAY_TOKEN = env("TELEGRAM_RELAY_TOKEN", "").strip()
TELEGRAM_RELAY_BIND_TOKEN = env("TELEGRAM_RELAY_BIND_TOKEN", TELEGRAM_RELAY_TOKEN).strip()
...
AI_RECOMMENDATIONS_SERVICE_URL = env("AI_RECOMMENDATIONS_SERVICE_URL", "http://127.0.0.1:8080").strip()
AI_RECOMMENDATIONS_SERVICE_TOKEN = env("AI_RECOMMENDATIONS_SERVICE_TOKEN", "").strip()
AI_RECOMMENDATIONS_SIGNING_SECRET = env("AI_RECOMMENDATIONS_SIGNING_SECRET", "").strip()
AI_RECOMMENDATIONS_POLL_INTERVAL_SECONDS = int(env("AI_RECOMMENDATIONS_POLL_INTERVAL_SECONDS", "10"))
AI_RECOMMENDATIONS_MAX_POLL_ATTEMPTS = int(env("AI_RECOMMENDATIONS_MAX_POLL_ATTEMPTS", "60"))
